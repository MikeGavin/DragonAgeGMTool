...
namespace Minion
{
    public class EcotPC : INotifyPropertyChanged, Minion.IEcotPC
    {
        ...
        public EcotPC(IPAddress ipaddress)
        {
            ...
            //pullall.CommandText = "SELECT * FROM EcotPC";
            ...
            //    while (reader.Read())
            //        CommandList.Add( new Tool.PCCommand() { Name = reader["Name"].ToString(), Tool = reader["Tool"].ToString(), Command = reader["Command"].ToString() });
            //    minion.Close();
